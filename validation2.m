...
% List of J values to test
J_values = [81, 41, 21];

% Loop over J_values
for J_index = 1:length(J_values)
    J = J_values(J_index);
    dx=L/(J-1); %number of points/string, space step
    ...
    % Plot the soundwave for the current J value
    figure;
    plot(tsave(1:count), S(1:count));
    xlabel('Time (s)');
    ylabel('Amplitude');
    title(sprintf('Soundwave for J = %d', J));
end
